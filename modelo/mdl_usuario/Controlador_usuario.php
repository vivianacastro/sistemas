...
class Controlador_usuario {
    ...
    /*
    public function crearCuentaUsuario()
    {
        $GLOBALS['mensaje'] = "";
        ...
        {   
            ...
            if($m->validarDatos($info['nombre_usuario'], $info['login'], $info['password'], $info['perfil']))
            {
                $m->crearCuentaUsuario($info['nombre_usuario'], $info['login'], $info['password'], $info['perfil'], $info["correo"], $info["telefono"], $info["extension"]);
    ...
    public function crear_nuevo_usuario(){
        ...
        $data = array('mensaje' => 
            'Crear nueva cuenta de usuario para acceder al sistema.',);

        $v = new Controlador_vista();

        $v->retornar_vista($_SESSION['perfil'],USUARIO, OPERATION_NEW_USER, $data);
    }
    ...
    public function eliminar_usuario_autorizado_adm_sistema() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($_SESSION['perfil'] == 'super') {
                $respuesta = $m->eliminarUsuarioAutorizado($data);
            }
    ...
    public function modificar_perfil_usuario_autorizado_adm_sistema() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($_SESSION['perfil'] == 'super') {
                $info = json_decode($_POST['jObject'], true);
                $rslt = $m->modificarPerfilUsuarioAutorizado($info['id'], 
                        $info['perfil']);
            }
