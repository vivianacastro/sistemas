...
class controlador_creacion
{
    ...
    **/
    public function guardar_planos_espacio(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            for ($i=0; $i < count($info); $i++) {
                $file = $info['archivo'.$i];
				$espacios = $infoEspacio['id_espacio'];
                if (count($espacios) == 1) {
                    $verificar = $m->guardarPlanoEspacio($infoEspacio['nombre_sede'],$infoEspacio['nombre_campus'],$infoEspacio['nombre_edificio'],$espacios,$file);
                }else{
    ...
    public function guardar_fotos_espacio(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            for ($i=0; $i < count($info); $i++) {
				...
				$espacios = $infoEspacio['id_espacio'];
                if (count($espacios) == 1) {
                    $verificar = $m->guardarFotoEspacio($infoEspacio['nombre_sede'],$infoEspacio['nombre_campus'],$infoEspacio['nombre_edificio'],$espacios,$file);
                }else{
    				for ($a=0; $a<count($espacios); $a++) {
    ...
    **/
    public function guardar_aire_acondicionado(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($verificar){
                $verificar = $m->guardarAire($info['numero_inventario'],$info['sede'],$info['campus'],$info['edificio'],$info['piso'],$info['espacio'],$info['capacidad'],$info['marca'],$info['modelo'],$info['tipo'],$info['tecnologia_aire'],$info['fecha_instalacion'],$info['instalador'],$info['periodicidad_mantenimiento'],$info['ubicacion_condensadora'],$info['responsable']);
            }
        }
        $result['mensaje'] = $GLOBALS['mensaje'];
        $result['sql'] = $GLOBALS['sql'];
    ...
    **/
    public function guardar_marca_inventario(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($verificar){
                $m->guardarMarcaInventario($info['nombre'],$info['bodega']);
            }
        }
        $result['mensaje'] = $GLOBALS['mensaje'];
        $result['sql'] = $GLOBALS['sql'];
    ...
    **/
    public function guardar_categoria(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            $info = json_decode($_POST['jObject'], true);
            $verificar = $m->verificarCategoria($info['nombre'],$info['bodega']);
            if($verificar){
    ...
    **/
    public function guardar_proveedor(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            $info = json_decode($_POST['jObject'], true);
            $verificar = $m->verificarProveedor($info['nombre'],$info['bodega']);
            if($verificar){
    ...
    **/
    public function guardar_articulo(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            $info = json_decode($_POST['jObject'], true);
            $verificar = $m->verificarArticulo($info['nombre_articulo'],$info['marca'],$info['categoria'],$info['bodega']);
            if($verificar){
