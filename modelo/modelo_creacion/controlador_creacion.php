    ...
    **/
    public function guardar_aire_acondicionado(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($verificar){
                $verificar = $m->guardarAire($info['numero_inventario'],$info['sede'],$info['campus'],$info['edificio'],$info['piso'],$info['espacio'],$info['capacidad'],$info['marca'],$info['tipo'],$info['tecnologia_aire'],$info['fecha_instalacion'],$info['instalador'],$info['periodicidad_mantenimiento'],$info['ubicacion_condensadora'],$info['responsable']);
            }
        }
        $result['mensaje'] = $GLOBALS['mensaje'];
        $result['sql'] = $GLOBALS['sql'];
