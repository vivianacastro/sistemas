...
class controlador_modificacion{
    ...
    public function modificar_cancha(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            $info = json_decode($_POST['jObject'], true);
            $verificar = $m->modificarCancha($info['id_sede'],$info['id_campus'],$info['id'],$info['uso'],$info['material_piso'],$info['tipo_pintura'],$info['longitud_demarcacion'],$info['$lat'],$info['$lng']);
        }
        $result['mensaje'] = $GLOBALS['mensaje'];
        $result['sql'] = $GLOBALS['sql'];
        $result['verificar'] = $verificar;
