    ...
    public function modificar_espacio(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (strcasecmp($info['uso_espacio'],'1') == 0) { //Salón
                $m->guardarSalon($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['capacidad'],$info['punto_videobeam']);
            }else if (strcasecmp($info['uso_espacio'],'2') == 0) { //Auditorio
                $m->guardarAuditorio($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['capacidad'],$info['punto_videobeam']);
            }else if (strcasecmp($info['uso_espacio'],'3') == 0) { //Laboratorio
                $m->guardarLaboratorio($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['capacidad'],$info['punto_videobeam'],$info['cantidad_puntos_hidraulicos'],$info['tipo_punto_sanitario'],$info['tipo_punto_sanitario_anterior'],$info['cantidad_puntos_sanitarios'],$info['cantidad_puntos_sanitarios_anterior']);
            }else if (strcasecmp($info['uso_espacio'],'4') == 0) { //Sala de Cómputo
                $m->guardarSalaComputo($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['capacidad'],$info['punto_videobeam']);
            }else if (strcasecmp($info['uso_espacio'],'5') == 0) { //Oficina
                $m->guardarOficina($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['punto_videobeam']);
            }else if (strcasecmp($info['uso_espacio'],'6') == 0) { //Baño
                $m->guardarBano($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['tipo_inodoro'],$info['cantidad_inodoro'],$info['tipo_orinal'],$info['tipo_orinal_anterior'],$info['cantidad_orinal'],$info['cantidad_orinal_anterior'],$info['tipo_lavamanos'],$info['tipo_lavamanos_anterior'],$info['cantidad_lavamanos'],$info['cantidad_lavamanos_anterior'],$info['ducha'],$info['lavatraperos'],$info['cantidad_sifones'],$info['tipo_divisiones'],$info['material_divisiones']);
            }else if (strcasecmp($info['uso_espacio'],'7') == 0) { //Cuarto Técnico
                $m->guardarCuartoTecnico($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red'],$info['punto_videobeam']);
            }else if (strcasecmp($info['uso_espacio'],'8') == 0) { //Bodega/Almacen
                $m->guardarBodega($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }else if (strcasecmp($info['uso_espacio'],'10') == 0) { //Cuarto de Plantas
                $m->guardarCuartoPlantas($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }else if (strcasecmp($info['uso_espacio'],'11') == 0) { //Cuarto de Aires Acondicionados
                $m->guardarCuartoAireAcondicionado($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }else if (strcasecmp($info['uso_espacio'],'12') == 0) { //Área Deportiva Cerrada
                $m->guardarAreaDeportivaCerrada($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }else if (strcasecmp($info['uso_espacio'],'14') == 0) { //Centro de Datos/Teléfono
                $m->guardarCentroDatos($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }else if (strcasecmp($info['uso_espacio'],'17') == 0) { //Cuarto de Bombas
                $m->guardarCuartoBombas($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_hidraulicos'],$info['tipo_punto_sanitario'],$info['tipo_punto_sanitario_anterior'],$info['cantidad_puntos_sanitarios'],$info['cantidad_puntos_sanitarios_anterior']);
            }else if (strcasecmp($info['uso_espacio'],'19') == 0) { //Cocineta
                $m->guardarCocineta($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_hidraulicos'],$info['tipo_punto_sanitario'],$info['tipo_punto_sanitario_anterior'],$info['cantidad_puntos_sanitarios'],$info['cantidad_puntos_sanitarios_anterior']);
            }else if (strcasecmp($info['uso_espacio'],'20') == 0) { //Sala de Estudio
                $m->guardarSalaEstudio($info['id'],$info['id_sede'],$info['id_campus'],$info['id_edificio'],$info['cantidad_puntos_red']);
            }
        }
        $result['mensaje'] = $GLOBALS['mensaje'];
        $result['sql'] = $GLOBALS['sql'];
